...
:class:`EvaluatorResult`. No single signal determines the final relative
strength: the weighted evaluator blends every available signal and quantifies
how much the signals agree, flagging conflict when they diverge.
"""

from __future__ import annotations

from backend.analytics.models import TrendConfig
from backend.analytics.relative_strength.models import EvaluatorResult, SignalOutput
...
class WeightedEvaluator:
    ...
    def evaluate(
        ...
        Args:
            ...
            config:  Relative-strength configuration (provides weights).
        ...
        weights = [
            getattr(config, _WEIGHT_KEYS.get(o.name, "weight_benchmark"))
            for o in available
        ]
