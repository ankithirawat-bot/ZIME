...
:class:`EvaluatorResult`. No single signal determines the final momentum: the
weighted evaluator blends every available signal and quantifies how much the
signals agree, flagging conflict when they diverge.
"""

from __future__ import annotations

from backend.analytics.models import TrendConfig
from backend.analytics.momentum.models import EvaluatorResult, SignalOutput

_WEIGHT_KEYS = {
    "roc": "weight_roc",
    "acceleration": "weight_acceleration",
    "persistence": "weight_momentum_persistence",
...
class WeightedEvaluator:
    ...
    def evaluate(
        ...
        Args:
            ...
            config:  Momentum configuration (provides weights).
        ...
        weights = [
            getattr(config, _WEIGHT_KEYS.get(o.name, "weight_roc")) for o in available
        ]
